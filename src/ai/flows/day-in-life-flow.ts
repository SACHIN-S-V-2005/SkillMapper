...
const DayInLifeOutputSchema = z.object({
    timeline: z.array(
        z.object({
            ...
            icon: z.enum(['Coffee', 'Code', 'Users', 'Presentation', 'Brainstorm', 'BarChart', 'Lunch', 'FileText', 'MessageSquare']).describe('An icon name that best represents the activity.'),
        })
...
const dayInLifePrompt = ai.definePrompt({
  ...
  prompt: `
    ...
    **Instructions:**
    ...
    3.  **Icon Mapping:** Assign one of the following icons to each activity: 'Coffee', 'Code', 'Users' (for meetings), 'Presentation', 'Brainstorm', 'BarChart' (for analytics/data), 'Lunch', 'FileText' (for documentation), 'MessageSquare' (for communication).
